<?php echo view_render_event('bagisto.admin.catalog.product.edit_form_accordian.videos.before', ['product' => $product]); ?>


<accordian :title="'<?php echo e(__('admin::app.catalog.products.videos')); ?>'" :active="false">
    <div slot="body">

        <?php echo view_render_event('bagisto.admin.catalog.product.edit_form_accordian.videos.controls.before', ['product' => $product]); ?>


        <div class="control-group <?php echo $errors->has('videos.*') ? 'has-error' : ''; ?>">
            <label><?php echo e(__('admin::app.catalog.products.video')); ?></label>

            <product-video></product-video>

            <span class="control-error" v-if="<?php echo $errors->has('videos.*'); ?>">
                <?php $count=1 ?>
                <?php $__currentLoopData = $errors->get('videos.*'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $message): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php echo str_replace($key, 'Video'.$count, $message[0]); $count++ ?>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </span>
        </div>

        <?php echo view_render_event('bagisto.admin.catalog.product.edit_form_accordian.videos.controls.after', ['product' => $product]); ?>


    </div>
</accordian>

<?php echo view_render_event('bagisto.admin.catalog.product.edit_form_accordian.videos.after', ['product' => $product]); ?>
...
<?php $__env->startPush('scripts'); ?>
    ...
    <script type="text/x-template" id="product-video-template">
        <div>
            ...
            <label class="btn btn-lg btn-primary" style="display: inline-block; width: auto" @click="createFileType">
                <?php echo e(__('admin::app.catalog.products.add-video-btn-title')); ?>
    ...
    <script type="text/x-template" id="product-video-item-template">
        <label class="image-item" v-bind:class="{ 'has-image': videoData.length > 0 }">
            ...
            <video class="preview" v-if="videoData.length > 0" width="200" height="160" controls>
                ...
                <?php echo e(__('admin::app.catalog.products.not-support-video')); ?>

            </video>

            <label class="remove-image" @click="removeVideo()">
                <?php echo e(__('admin::app.catalog.products.remove-video-btn-title')); ?>
    ...
    <script>
        Vue.component('product-video', {
            ...
            data: function() {
                return {
                    videos: <?php echo json_encode($product->videos, 15, 512) ?>,
...
<?php $__env->stopPush(); ?><?php /**PATH /var/www/html/bagisto/packages/Webkul/Admin/src/Providers/../Resources/views/catalog/products/accordians/videos.blade.php ENDPATH**/ ?>